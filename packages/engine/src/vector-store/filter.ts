...
type OperatorValueMap<
  ...
  ValueMap extends Record<string, any> = any,
> = {
  $eq: FilterValue;
  $ne: FilterValue;
...
> =
  | FilterCondition<Op, ValueMap, LogicalValueMap, Blacklisted, FieldValue>
  | null
  | undefined;
...
type FilterCondition<
  ...
  FieldValue = VectorFieldValue,
> = (
  | FieldCondition<Op, ValueMap, FieldValue>
  | LogicalCondition<Op, ValueMap, LogicalValueMap>
) &
...
abstract class BaseFilterTranslator<Filter = VectorFilter, Result = Filter> {
  ...
  protected isLogicalOperator(key: string): key is LogicalOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.logical.includes(
      key as LogicalOperator,
    );
  }

  protected isBasicOperator(key: string): key is BasicOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.basic.includes(
      key as BasicOperator,
    );
  }

  protected isNumericOperator(key: string): key is NumericOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.numeric.includes(
      key as NumericOperator,
    );
  }

  protected isArrayOperator(key: string): key is ArrayOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.array.includes(
      key as ArrayOperator,
    );
  }

  protected isElementOperator(key: string): key is ElementOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.element.includes(
      key as ElementOperator,
    );
  }

  protected isRegexOperator(key: string): key is RegexOperator {
    return BaseFilterTranslator.DEFAULT_OPERATORS.regex.includes(
      key as RegexOperator,
    );
  }
  ...
  protected static readonly ErrorMessages = {
    ...
    INVALID_TOP_LEVEL_OPERATOR: (op: string) =>
      `Invalid top-level operator: ${op}`,
  ...
  private validateFilterSupport(
    ...
    if (Array.isArray(node)) {
      const arrayResults = node.map((item) =>
        this.validateFilterSupport(item, path),
      );
    ...
    for (const [key, value] of Object.entries(nodeObj)) {
      ...
      if (this.isOperator(key)) {
        if (!this.isValidOperator(key)) {
          isSupported = false;
          messages.push(
            BaseFilterTranslator.ErrorMessages.UNSUPPORTED_OPERATOR(key),
          );
          continue;
        }
        ...
        if (!path && !this.isLogicalOperator(key)) {
          isSupported = false;
          messages.push(
            BaseFilterTranslator.ErrorMessages.INVALID_TOP_LEVEL_OPERATOR(key),
          );
          continue;
        }
        ...
        if (
          ...
          isSupported = false;
          messages.push(
            BaseFilterTranslator.ErrorMessages.ELEM_MATCH_REQUIRES_OBJECT,
          );
          continue;
        }
        ...
        if (this.isLogicalOperator(key)) {
          if (key === "$not") {
            if (Array.isArray(value) || typeof value !== "object") {
              isSupported = false;
              messages.push(
                BaseFilterTranslator.ErrorMessages.NOT_REQUIRES_OBJECT,
              );
              continue;
            }
            if (this.isEmpty(value)) {
              isSupported = false;
              messages.push(
                BaseFilterTranslator.ErrorMessages.NOT_CANNOT_BE_EMPTY,
              );
              continue;
            }
          ...
          if (Array.isArray(value)) {
            ...
            if (hasDirectOperators) {
              isSupported = false;
              messages.push(
                BaseFilterTranslator.ErrorMessages.INVALID_LOGICAL_OPERATOR_CONTENT(
                  newPath,
                ),
              );
              continue;
