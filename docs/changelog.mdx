...
The [playground](/search-and-retrieval/playground) and [hosted](/production/hosting-ui) chats now use agentic search. Instead of retrieving once before answering, the model plans its approach and searches your namespace in a tool-calling loop until it has enough context. The playground shows the full step-by-step trace for debugging, while the hosted interface shows end users a compact progress indicator.
