...
Instead of making making the search yourself, let the model use Agentset as a search tool. The model decides what to search for, and follow-up with additional queries as necessary. 

This handles multi-part questions, follow-ups, and ambiguous queries better than [simple RAG](/search-and-retrieval/simple-rag), which retrieves only once before generating.
